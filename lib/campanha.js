...
export const JANELA_INICIO = 9.75; // 09:45
...
//
// 40 msgs em 4h15 (255min) exige ~1 a cada 6,4min na média — parece folgado,
// mas o cron só decide a cada 5min (grid fixo), então na prática só existem
// dois pisos possíveis: 5min (deixa QUALQUER tick disparar, atinge 40) ou
// 10min+ (obriga pular pelo menos 1 tick, e o alcançável despenca pra ~25).
// Não existe meio-termo suave aqui — testado em simulação: piso 5 bate 39-40
// quase sempre, piso 5.1 já cai pra ~25. Decisão explícita do dono: manter
// meta 40, aceitando piso ~5min (mesmo intervalo do tick do cron, sem folga
// extra de segurança). Registrado aqui pra não parecer acidental depois.
