...
Entre 45 e 70 PALAVRAS no total, distribuídas em 4 ou 5 linhas curtas separadas por quebra de
linha (\\n). CONTE antes de responder. Menos que 45 vira legenda seca e não convence; mais que 70
vira textão que ninguém lê no WhatsApp e a pessoa arquiva sem abrir.

Cada linha é uma frase curta, do tamanho de quem digita no celular — não escreva parágrafo corrido.

Estrutura (não pule nenhum passo):
1. Cumprimenta pelo nome + se identifica como ${restaurante}, de forma leve. Use EXATAMENTE o
   nome informado abaixo e nada além dele — nunca acrescente sobrenome. Se vier "(desconhecido)",
   cumprimente sem citar nome nenhum ("Oi, tudo bem?").
2. Uma frase conectando fome/pausa curta de almoço com o momento — escolhe UM jeito natural de
   dizer isso (varie a cada geração), sem listar vários cenários possíveis.
3. Um detalhe sensorial concreto da marmita (cheiro, tempero, comida quentinha feita na hora) —
   não só "é bom", descreve algo específico que dá vontade de comer. Aponta pro vídeo que
   está junto, pra pessoa ver a comida de verdade.
4. Fecha citando ${brinde} de brinde e o código do cupom, do jeito que fizer sentido pro contexto
   de relação com a pessoa (primeira vez / voltar a topar / agradecimento por já ser cliente),
   com uma chamada clara pra pedir agora.
...
- fugir da faixa de 45-70 palavras — isso é regra dura, não sugestão
