...
const client = axios.create({
  ...
  // Sem timeout o axios espera pra sempre: a função ficava presa até a Vercel
  // matá-la aos 60s, o pg_net desistia antes disso e o log não registrava causa
  // nenhuma — só uma campanha parada sem explicação. Uma geração de copy leva
  // ~4s; passou de 25s não vai completar dentro da janela de qualquer jeito.
  timeout: 25000,
});
...
// alta ("Amanda Ribeiro❤️", "odety souza", "Nando "). No áudio, o modelo lia o
// nome inteiro — "Oi, Peterson Lincon do Chapelão aqui" soa como cobrança de
// banco, não como vizinho chamando. Só o primeiro nome resolve.
...
export function primeiroNome(nome) {
  ...
  if (!primeiro || NOMES_GENERICOS.has(primeiro.toLowerCase())) return null;

  // "odety" → "Odety", "MULTLUB" → "Multlub": o TTS enfatiza caixa alta como
  // grito, então normalizar a caixa também melhora o áudio. Cada parte de um
  // nome composto entra maiúscula, senão "Ana-Clara" volta como "Ana-clara".
  return primeiro
    .toLowerCase()
...
Dados do cliente:
- Nome: ${primeiroNome(cliente.nome) || '(desconhecido — cumprimente sem citar nome)'}
...
// Sem isso o erro que chega no painel é o genérico do axios ("Request failed
// with status code 429"), que não diz nem qual serviço recusou nem por quê —
// e a rota chama OpenAI, ElevenLabs e Evolution em sequência.
function erroOpenAI(err) {
  ...
  if (status === 429) {
    const cota = resp?.code === 'insufficient_quota';
    detalhe = cota
  ...
  if (status === 401) {
    detalhe = `chave de API recusada (${detalhe}). Confira OPENAI_API_KEY.`;
  }
...
// A campanha deixou de mandar áudio, então o texto virou a mensagem inteira em
// vez de uma legenda de apoio — daí um schema de campo único, e não {audio, cta}.
// gerarAudioECta continua existindo pra demo de prospecção, que mantém o áudio:
// lá ele é justamente o que impressiona o prospect na demonstração ao vivo.
async function gerarMensagemUnica(prompt) {
  let data;
  try {
    ({ data } = await client.post('/chat/completions', {
      model: process.env.OPENAI_MODEL || 'gpt-4o',
  ...
  // Colapsar só espaço e tabulação: a quebra de linha é o que dá o respiro de
  // mensagem de WhatsApp, e um \s{2,} genérico esmagaria tudo num parágrafo só.
...
// no sistema, só a tag), então a âncora é sempre o vídeo real da marmita
// sendo montada + o brinde. O que muda por público é só a RELAÇÃO com a casa (ver `segmento`
...
// Sequência de recompra: etapa 0 é o primeiro toque; 1 e 2 só disparam se a
// pessoa não converteu na etapa anterior (ver campanha_selecionar_leads).
// Sem esse contexto, um "lembrete" soa como se fosse a primeira mensagem de
// novo, e a "última chance" não carrega nenhuma urgência real.
const CONTEXTO_POR_ETAPA = {
  0: '',
  1: `\n## ESTA É A 2ª TENTATIVA (lembrete)
A pessoa recebeu esse convite há alguns dias e não respondeu. Reconheça DE LEVE que é um lembrete
(ex: "separei de novo esse cupom pra você", "seu cupom ainda tá valendo") — não repita a mensagem
anterior do zero como se fosse a primeira vez, mas também não soe como cobrança ou insistência.`,
  2: `\n## ESTA É A ÚLTIMA TENTATIVA DA SEQUÊNCIA (cupom expira em poucos dias)
É a última vez que esse convite específico volta — depois disso a pessoa não recebe mais esse
brinde. Comunique urgência real e final no fechamento (ex: "esse é o último cupom que eu separo,
já expira em poucos dias"), mantendo o tom caloroso — urgência de verdade, não pressão agressiva.`,
};
...
export async function gerarMensagemPrimeiraCompra({
  ...
  // O que separa "mensagem de hoje" de "disparo automático" é citar algo que só
  // vale hoje. Chuva e frio, especialmente, empurram exatamente a decisão que a
  // campanha quer: não sair pra comer na rua.
  const blocoContextoDoDia = contextoDoDia
    ? `\n## CONTEXTO REAL DE HOJE (use, é o que faz a mensagem parecer escrita agora)
${contextoDoDia}

Amarre isso à comida na linha 2, de forma natural e curta — é o gancho mais forte que você tem.
Não force nem transforme em duas frases: continua sendo UMA linha. E não invente nada além do
que está escrito acima sobre o dia.`
    : '';

  const prompt = `Você escreve uma mensagem de TEXTO de WhatsApp para um restaurante de comida
caseira: ${restaurante}, em ${cidade}. A mensagem vai junto com o VÍDEO REAL da marmita sendo
montada na cozinha, como legenda dele.

Esta mensagem é TUDO que a pessoa recebe além do vídeo — não existe áudio nem outra mensagem
depois. Ela sozinha precisa cumprimentar, dar o contexto, despertar vontade e fechar com a oferta.
...
## TAMANHO — RESTRIÇÃO MAIS IMPORTANTE DESTE PROMPT
Entre 25 e 35 PALAVRAS no total, distribuídas em 3 linhas curtas separadas por quebra de
linha (\\n). CONTE antes de responder. Isso é uma legenda de vídeo no WhatsApp, não um bilhete —
tem que dar pra ler inteira de relance, sem rolar nada. Mais que 35 já é longa demais.

Cada linha é uma frase curta, do tamanho de quem digita no celular — não escreva parágrafo corrido.

Estrutura (3 linhas, não pule nenhum passo):
1. Cumprimenta pelo nome + se identifica como ${restaurante} numa frase só, curta. Use EXATAMENTE
   o nome informado abaixo e nada além dele — nunca acrescente sobrenome. Se vier "(desconhecido)",
   cumprimente sem citar nome nenhum ("Oi, tudo bem?").
2. Uma frase só juntando o momento (fome/pausa de almoço) com UM detalhe sensorial concreto da
   marmita (cheiro, tempero, comida quentinha) — não duas frases separadas, é uma coisa só, direto.
3. Fecha citando ${brinde} de brinde e o código do cupom, do jeito que fizer sentido pro contexto
   de relação com a pessoa (primeira vez / voltar a topar / agradecimento por já ser cliente),
   com uma chamada clara e curta pra pedir agora.

Tom: humano, caloroso, direto — gente do bairro mandando mensagem, não telemarketing, não anúncio.
Linguagem falada ("tá", "pra", "cê", "né"), mas OBJETIVA: cada palavra precisa ganhar seu lugar.

PROIBIDO:
- "sentimos sua falta", "matar a saudade", "faz tempo que você não..."
- a palavra "buffet" — o vídeo é da marmita sendo montada na cozinha, não uma mesa de buffet
...
- fugir da faixa de 25-35 palavras — isso é regra dura, não sugestão
- audio tags entre colchetes tipo [warm] — isto é TEXTO que a pessoa lê, não roteiro de áudio
- CAIXA ALTA pra dar ênfase, e mais de um ponto de exclamação seguido

## FORMATAÇÃO DE WHATSAPP
- Quebra de linha (\\n) entre as frases é o que dá o respiro da mensagem. Use.
- No máximo 2 emoji na mensagem inteira, e só onde acrescentam — mensagem salpicada de emoji
  parece disparo em massa, que é exatamente o que não queremos aparentar.
- Pode usar *negrito do WhatsApp* (asteriscos) em UMA coisa só: o código do cupom.

Dados:
- Nome da pessoa: ${primeiroNome(cliente.nome) || '(desconhecido — cumprimente sem citar nome)'}
...
"mensagem": a mensagem completa de WhatsApp, com as quebras de linha como \\n.

Responda APENAS com o JSON plano {"mensagem": "..."}, sem markdown, sem explicação.`;

  return { mensagem: await gerarMensagemUnica(prompt) };
}
