...
acompanha o VÍDEO REAL da marmita sendo montada na cozinha, que é enviado junto.
...
## DURAÇÃO — RESTRIÇÃO MAIS IMPORTANTE DESTE PROMPT
O áudio precisa ficar entre 20 e 26 SEGUNDOS de fala, nem mais nem menos. Isso equivale a
75-85 PALAVRAS no roteiro inteiro (as palavras dentro de audio tags como [warm] não contam) —
CONTE de verdade antes de responder, letra por letra se precisar: a tendência natural é escrever
curto demais e ficar bem abaixo disso. Se o rascunho mental tiver menos de 75 palavras, ACRESCENTE
mais um detalhe sensorial ou uma frase de transição antes de fechar — não force encaixar tudo em
3 frases telegráficas. Menos que 75 soa cortado; mais que 85 estoura o tempo.

Estrutura do áudio (5 frases curtas, cada uma puxa a próxima — não pule nenhuma, é isso que
preenche a faixa de palavras certa):
...
3. Um detalhe sensorial concreto da marmita (cheiro, tempero, comida quentinha feita na hora) —
   não só "é bom", descreve algo específico que dá vontade de comer.
4. Menciona que o vídeo da marmita sendo montada vai junto, pra pessoa ver a comida de verdade.
5. Fecha citando ${brinde} de brinde, do jeito que fizer sentido pro contexto de relação com a
   pessoa (primeira vez / voltar a topar / agradecimento por já ser cliente).

Tom: humano, caloroso, direto — gente do bairro convidando rápido, não telemarketing, não anúncio,
e não uma conversa longa. Linguagem falada ("tá", "pra", "cê", "né"), mas OBJETIVA: cada palavra
precisa ganhar seu lugar no roteiro curto.
...
- fugir da faixa de 75-85 palavras no roteiro do áudio — isso é regra dura, não sugestão

## COMO ESCREVER PRO ÁUDIO SOAR HUMANO
O texto é lido pelo ElevenLabs v3, que INTERPRETA marcações de atuação:
- Use no máximo 2 audio tags entre colchetes (roteiro curto não precisa de mais que isso), antes
  da frase que elas afetam. Úteis: [warm] [friendly] [thoughtful] [excited] [reassuring].
- Reticências (...) só onde precisar de UMA pausa natural — não em toda frase, senão estica o áudio.
...
"cta": uma linha curtíssima (máximo 14 palavras) que acompanha o vídeo da marmita: o brinde + o código
do cupom + chamada pra pedir. É TEXTO puro, sem audio tags, pode ter emoji. NÃO use a palavra "buffet".
Exemplo de estilo: "🍱 Marmita fresquinha! Ganhe ${brinde} — cupom ${cupom.codigo}"
