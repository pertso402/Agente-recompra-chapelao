...
// Timeout generoso mas finito: enviar mídia exige que a Evolution baixe o vídeo
// e suba pro WhatsApp (~4s em condição normal), mas quando o servidor engasga
// a requisição fica pendurada e leva a função inteira junto.
...
function erroEvolution(err, acao, instancia = '?') {
  ...
  // A Evolution devolve 400 em dois casos que não têm nada a ver entre si:
  // sessão caída e número que simplesmente não existe no WhatsApp (o corpo vem
  // com "exists": false). Tratar todo 400 como queda de instância mandava
  // procurar QR code quando o problema era um telefone errado no cadastro.
  const numeroInexistente = /"exists"\s*:\s*false/i.test(detalhe);

  // 428 (Precondition Required) e "Connection Closed" são os sinais de sessão
  // caída de verdade — sessão expirada, celular offline, container reiniciado.
  // Aí a chave e a instância estão certas e o erro cru não sugere em momento
  // nenhum o que de fato resolve: reconectar lendo o QR code.
  const desconectada =
    !numeroInexistente && (status === 428 || /connection closed|connection lost/i.test(detalhe));

  let mensagem;
  if (desconectada) {
    mensagem = `instância "${instancia}" parece desconectada do WhatsApp — reconecte lendo o QR code no painel da Evolution. Detalhe: ${detalhe}`;
  } else if (numeroInexistente) {
    mensagem = `número não existe no WhatsApp (confira o cadastro; a instância "${instancia}" está normal). Detalhe: ${detalhe}`;
  } else {
    mensagem = detalhe;
  }

  const e = new Error(`Evolution (${acao}): ${mensagem}`);
  e.status = status;
  e.instancia = instancia;
  e.desconectada = desconectada;
  e.numeroInexistente = numeroInexistente;
  e.original = err;
  return e;
}
...
// Ciclo de vida real de uma mensagem no WhatsApp. A Evolution acumula TODOS os
// ACKs recebidos numa lista — fora de ordem, repetidos, e com "EDITED" no meio,
// que não é estágio de entrega nenhum. Por isso o que vale é o estágio mais
// avançado já alcançado, nunca o último item da lista.
const RANK_ACK = { ERROR: 0, PENDING: 1, SERVER_ACK: 2, DELIVERY_ACK: 3, READ: 4, PLAYED: 5 };
...
const ACK_PARA_STATUS = {
  ...
  READ: 'lida',
  PLAYED: 'lida', // áudio ouvido implica mensagem aberta
};

export function statusDeAcks(acks) {
  let melhor = null;
  for (const item of acks || []) {
    const ack = item?.status;
    if (RANK_ACK[ack] === undefined) continue;
    if (melhor === null || RANK_ACK[ack] > RANK_ACK[melhor]) melhor = ack;
  }
  return melhor ? { ack: melhor, status: ACK_PARA_STATUS[melhor] } : null;
}

// Consulta o ACK de uma mensagem já enviada. Usa findMessages em vez de webhook
// de propósito: a instância de disparo já tem webhook apontado pro agente de
// atendimento, e redirecioná-lo pra cá arriscaria deixar o número sem quem
// responda as mensagens que chegam.
export async function consultarStatusMensagem(messageId, origem = 'recompra') {
  if (!messageId) return null;
  const { client, instancia } = resolver(origem);
  try {
    const { data } = await client.post(`/chat/findMessages/${instancia}`, {
      where: { key: { id: messageId } },
    });
    const registro = data?.messages?.records?.[0];
    if (!registro) return null;
    return statusDeAcks(registro.MessageUpdate);
  } catch (err) {
    throw erroEvolution(err, 'consultarStatus', instancia);
  }
}
